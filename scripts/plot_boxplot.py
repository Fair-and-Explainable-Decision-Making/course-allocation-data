...
metrics = ["seats", "nash", "zeros", "PMMS_violations", "EF_violations", "EF1_violations", "runtime"]
metric_names = ["Seats","NSW", "Empty Bundles", "PMMS violations", "EF violations", "EF-1 violations", "Runtime (s)"]
...
for num,metric in enumerate(metrics):
    ...
    for i, col in enumerate(pivot_df.columns):
        ...
        names.append(col)
        xs.append(np.random.normal(i +0.65, 0.03, pivot_df[col].values.shape[0]))  # adds jitter to the data points - can be adjusted

    box = axs[plt_num].boxplot(vals, labels=names, patch_artist=True, widths=0.35)


    for whisker, cap, color in zip(box['whiskers'], box['caps'], [palette[0], palette[0], palette[1], palette[1], palette[2], palette[2]]):
    ...
    if higher[num]==1:
        axs[plt_num].text(0.3, 0.94, "↑ Higher is better", ha='center', va='center', transform=axs[plt_num].transAxes,
            fontsize=9, bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.5'))
    elif num==6:
        axs[plt_num].text(0.3, 0.94, "↓ Lower is better", ha='center', va='center', transform=axs[plt_num].transAxes,
            fontsize=9, bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.5'))

    else:
        axs[plt_num].text(0.715, 0.94, "↓ Lower is better", ha='center', va='center', transform=axs[plt_num].transAxes,
            fontsize=9, bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.5'))
...
legend_elements = [Patch(facecolor=faded_palette[0], edgecolor=palette[0],
                         label='Serial Dictatorship'),
                    Patch(facecolor=faded_palette[1], edgecolor=palette[1],
                         label='Round Robin'),
                    Patch(facecolor=faded_palette[2],edgecolor=palette[2],
                         label='Yankee Swap')]
axs[3].legend(handles=legend_elements,  ncol=1, bbox_to_anchor=(-0.05, 0.3), loc='upper left', fontsize=12)


plt.tight_layout()
plt.savefig(f"./experiments/boxplot.jpg", dpi=300)
plt.savefig(f"./experiments/boxplot.pdf", format="pdf",dpi=300)
print(pivot_df)
